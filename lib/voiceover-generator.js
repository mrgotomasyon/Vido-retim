...
// Params matched EXACTLY to the confirmed working example:
// {"stability":0.45,"similarity_boost":0.80,"style":0.70,"speed":0.95}
// Voice: tnSpp4vdxKPjI9w0GnoV  Language: tr
// ---------------------------------------------------------------------------
...
// Same voice and params as KIE path for consistent output quality.
// ---------------------------------------------------------------------------

const ELEVENLABS_VOICE_ID = process.env.KIE_VOICE_ID || "tnSpp4vdxKPjI9w0GnoV";
...
// ---------------------------------------------------------------------------
// Windows SAPI PowerShell fallback (Windows only — local dev)
// ---------------------------------------------------------------------------

function generateWithSapi({ text, outputPath, culture, rate }) {
  if (process.platform !== "win32") {
    return Promise.reject(new Error(
      "TTS API anahtarı bulunamadı. KIE_API_KEY veya ELEVENLABS_API_KEY tanımlı olmalı."
    ));
  }
...
// ---------------------------------------------------------------------------
// Main entry point — priority: KIE TTS → ElevenLabs → SAPI (Windows only)
// ---------------------------------------------------------------------------

async function generateVoiceover({ text, outputPath, culture = "tr-TR", rate = -2, skipRewrite = false }) {
  // Ham metni TTS-uyumlu senaryoya çevir (GPT-4o → rules fallback)
  // skipRewrite=true: metin zaten temizlenmiş (produce-video tarafından önceden yapıldı)
  const ttsText = skipRewrite ? text : await writeScriptForTTS(text);
  console.log("[Voiceover] TTS metni:", ttsText.slice(0, 120) + (ttsText.length > 120 ? "…" : ""));
  ...
  // 1. KIE ElevenLabs Turbo 2.5 — best quality
  if (kieKey) {
    try {
      result = await generateWithKieTTS({ text: ttsText, outputPath, apiKey: kieKey });
    } catch (err) {
      console.warn("[Voiceover] KIE TTS başarısız, bir sonraki deneniyor:", err.message);
    }
  }

  // 2. ElevenLabs direct API (same voice & params as KIE)
  if (!result && elevenKey) {
    try {
      result = await generateWithElevenLabs(ttsText, outputPath);
    } catch (err) {
      console.warn("[Voiceover] ElevenLabs direkt başarısız:", err.message);
    }
  }

  // 3. SAPI fallback — Windows only (local dev)
  if (!result) {
    result = await generateWithSapi({ text: ttsText, outputPath, culture, rate });
  }

  // ttsText'i döndür — produce-video.js ekran yazılarını bununla oluşturur
  result.ttsText = ttsText;
  return result;
}
