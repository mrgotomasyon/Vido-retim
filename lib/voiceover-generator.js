...
async function generateVoiceover({ text, outputPath, culture = "tr-TR", rate = -2 }) {
  ...
  if (kieKey) {
    try {
      return await generateWithKieTTS({ text: ttsText, outputPath, apiKey: kieKey });
    } catch (err) {
  ...
  if (elevenKey) {
    try {
      return await generateWithElevenLabs(ttsText, outputPath);
    } catch (err) {
  ...
  return generateWithSapi({ text: ttsText, outputPath, culture, rate });
}
