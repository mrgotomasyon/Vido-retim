...
const { getMusicBedPath }                                = require("./music-cache");
...
async function produceVideo({ text }) {
  ...
  // Karakter sayısına orantılı süre: uzun cümle → uzun süre (eşit bölünme yerine)
  const totalChars = sentences.reduce((sum, s) => sum + s.length, 0) || 1;
  let capT = 0;
  const captions = sentences.map((s, i) => {
    const { line1, line2 } = toLines(s);
    const dur = (s.length / totalChars) * voiceDur;
    const start = +capT.toFixed(3);
    capT += dur;
    const end   = +Math.min(capT, voiceDur + 0.3).toFixed(3);
    return { start, end, line1, line2: line2 || "", accent: i === 0 };
  });
  ...
  // Müzik: KIE Suno önbellekten al, yoksa template'den çıkar
  // her iki durumda da stream_loop ile video süresine looplar
  const musicPath = path.join(workDir, "music-bed.m4a");
  const cachedMusic = await getMusicBedPath();
  const musicSource = cachedMusic || TEMPLATE_BG;
  console.log(`[Müzik] Kaynak: ${cachedMusic ? "KIE Suno önbellek" : "template video"}`);
  await extractVideoAudio(musicSource, musicPath, videoDur);
