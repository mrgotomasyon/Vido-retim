...
async function produceVideo({ text }) {
  ...
  // +0.3s gecikme: ses söylenmeye başladıktan kısa süre sonra yazı çıkar
  const CAP_OFFSET = 0.3;
  const captions = sentences.map((s, i) => {
    const { line1, line2 } = toLines(s);
    return {
      start: +(i * sentenceDur + CAP_OFFSET).toFixed(3),
      end:   +Math.min((i + 1) * sentenceDur + CAP_OFFSET, voiceDur + 0.5).toFixed(3),
      line1,
      line2: line2 || "",
      accent: i === 0
    };
