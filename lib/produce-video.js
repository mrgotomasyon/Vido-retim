...
async function produceVideo({ text }) {
  ...
  // 2. Ekranda GÖRÜNECEK metni hesapla — toLines aynı kırpma kurallarını uygular
  const sentences = parseSentences(ttsText);
  const n = sentences.length;
  const hookLines  = toLines(sentences[0] || "ALGEONEX");
  const valueLines = toLines(sentences[Math.max(1, Math.floor(n / 2))] || sentences[0] || "ALGEONEX");

  // Ses = ekrandaki kelimeler (line1 + line2) — fazlası yok, eksik yok
  const hookDisplay  = [hookLines.line1,  hookLines.line2 ].filter(Boolean).join(" ");
  const valueDisplay = [valueLines.line1, valueLines.line2].filter(Boolean).join(" ");
  const voiceScript  = `${hookDisplay}. ${valueDisplay}. Ücretsiz AI analizi. algeonex nokta com.`;
