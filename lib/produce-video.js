...
async function produceVideo({ text }) {
  ...
  const voiceResult = await generateVoiceover({ text, outputPath: voicePath });
  // GPT-4o tarafından yeniden yazılan metin — ekran yazılarıyla ses eşleşsin
  const displayText = voiceResult.ttsText || text;
  const voiceDur = measureDuration(voicePath);
  const videoDur = Math.max(Math.ceil(voiceDur) + 1.5, 15);
  console.log(`[1/5] ✅ Ses: ${voiceDur.toFixed(2)}s → Video: ${videoDur.toFixed(1)}s (min 15s)`);

  // 2. Sahneler — TTS metni kullanılır (ses + ekran yazısı aynı olur)
  const platform = { name: "instagram_dynamic", width: 1080, height: 1920, duration: videoDur, fps: 30 };
  const scenes   = buildScenes(videoDur, displayText);
  console.log(`[2/5] 3 scene oluşturuldu: ${scenes.map(s=>s.line1).join(" | ")}`);
