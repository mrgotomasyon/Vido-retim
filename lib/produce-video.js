...
function buildScenes(totalDur, text) {
  ...
  // Metni cümlelere böl (emoji ve noktalama temizle)
  ...
  const toLines = (s) => {
    ...
    const MAX = 16; // max chars per line (safe for 78px font at 1080px width)
    if (c.length <= MAX) return { line1: c, line2: "" };
    // Fill line1 word-by-word up to MAX chars, rest goes to line2
    const ws = c.split(" ");
    let line1 = "";
    let split = ws.length; // default: all on line1
    ...
    // Truncate line2 to MAX chars at word boundary if needed
    if (line2.length > MAX) {
...
async function produceVideo({ text }) {
  ...
  // 2. Sahne cümlelerini seç — ekrandaki kartların metni
  const sentences = (ttsText || "")
    ...
    .split("\n").map(s => s.trim()).filter(Boolean);
  const n = sentences.length;
  const hookSentence  = sentences[0] || "ALGEONEX";
  const valueSentence = sentences[Math.max(1, Math.floor(n / 2))] || hookSentence;

  // Ses scripti: SADECE ekranda görünecek sahnelerin metni
  // Bu sayede ses ile ekran yazısı birebir eşleşir
  const voiceScript = `${hookSentence}. ${valueSentence}. Ücretsiz AI analizi için algeonex nokta com.`
    .replace(/\.\./g, ".");
  console.log(`[2/5] Ses scripti (sahne metinleri): "${voiceScript.slice(0, 100)}"`);

  // 3. TTS sesi üret — script zaten temiz, yeniden yazılmasın
  console.log("[3/5] TTS sesi üretiliyor...");
  const voicePath = path.join(RENDER_BASE, "voice", `${jobId}.mp3`);
  await generateVoiceover({ text: voiceScript, outputPath: voicePath, skipRewrite: true });
  const voiceDur = measureDuration(voicePath);
  const videoDur = Math.max(Math.ceil(voiceDur) + 1.5, 15);
  console.log(`[3/5] ✅ Ses: ${voiceDur.toFixed(2)}s → Video: ${videoDur.toFixed(1)}s (min 15s)`);

  // Sahneler — ttsText ile oluştur (ses scriptiyle aynı cümleler seçilir)
  ...
  console.log(`[Sahneler] ${scenes.map(s => s.line1 + (s.line2 ? " / " + s.line2 : "")).join(" | ")}`);

  const campaign = {
    voiceoverText: voiceScript,
    hook: `${scenes[0].line1} ${scenes[0].line2}`,
    scenes,
    // captions dizisi geçilmiyor → renderVideo, scenes üzerinden 3 büyük başlık kartı üretir
